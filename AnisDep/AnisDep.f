...
***********************************************************************************
* 	A FORTRAN-77 program for the computation of spatial variation factors
*	of teleseismic shear-wave splitting parameters for the purpose of 
*	estimating the optimal depth of the source of seismic anisotropy
*
*       Authors: Stephen S. Gao & Kelly H. Liu
*	sgao@mst.edu; liukh@mst.edu
*	Missouri University of Science and Technology
*	Date last modified: 9/22/2011
***********************************************************************************

	parameter(maxsm=100000) !! maximum number of splitting measurements
	parameter(dx1=0.05, dx2=0.4) !! block length in deg. 
	parameter(ddx=0.05)      !! distance between center of neighboring blocks
	parameter(dmove=0.05)  !! moving step size in deg.

	parameter(w1=1.0/180.0, w2=1.0/2) !! weighting factors for PHI and DT
	parameter(dep1=00.0, dep2=400.0, ddep=5.0) !! depth range and interval

	character  nin*50 !! input file name
	character nout*13 !! output file names


	character*3 nphase(maxsm)  !! phase name (e.g., SKS)
	real*4 stlat(maxsm), stlon(maxsm)  !! station latitude, longitude
	real*4 evlat(maxsm), evlon(maxsm),evdep(maxsm) !! event lat, lon, depth (in km)
	real*4 dt(maxsm), ph(maxsm)   !! splitting time & fast direction
	real*4 dtb(maxsm), phb(maxsm) !! dt and phi inside a block
	real*4 sigmadtph(maxsm) !! STD
	real*4 rlat(maxsm), rlon(maxsm) !! piercing points coordinates

** Ask for input data. Do not make the name longer than 50 characters!
	write(6,61)
61	format('Enter name of SWS measurements file: ',$)
	read(5,51) nin
51	format(a50)

* Number of block groups (each group has a uniform dx):
	ndx=(dx2-dx1)/ddx+1

** Start the loop for block groups (each has a different dx):
...
*output.dx0.05   !! Format of output file name
*1234567890123

        nout(1:9)='output.dx'

        open(9,file='tmp.name')
        write(9,91) dx
91      format(f4.2)
        close(9)
        open(9,file='tmp.name')
        read(9,92) nout(10:13)
92      format(a4)
        close(9)
        if(nout(13:13).eq.' ') nout(13:13)='0'

	open(8,file=nout)

** Read the observed splitting parameters into arrays
*

	open(1,file=nin)
	do i=1,maxsm
	read(1,*,end=111) stlon(i), stlat(i),evlon(i),
     &                    evlat(i),evdep(i),
     &		          ph(i), dt(i), nphase(i)
	enddo
111	close(1)
	...
	print*,'The number of measurements is ', nmea

**   Start the loop of assumed depth of anisotropy

	ndep=int((dep2-dep1)/ddep)+1

	do 9902 kk=1,ndep

	anisodep=dep1+(kk-1)*ddep 

	open(1,file='piercing.tmp')	

** For the kk-th anisodep, compute the geographic coordinates
*	of the ray-piercing points

	do i=1,nmea
	call distazsub(evlat(i), evlon(i), stlat(i), stlon(i),
     &		tmp, gcarc, baz, tmp)

	if(nphase(i).eq.'SKS') call tat_sks(gcarc,evdep(i),tmp,rayp)
	if(nphase(i).eq.'SKK') call tat_skks(gcarc,evdep(i),tmp,rayp)
	if(nphase(i).eq.'PKS') call tat_pks(gcarc,evdep(i),tmp,rayp)

* Piercing points location (rlon1,rlat1) of this ray path at anisodep

        call find_loc(anisodep, rayp, stlat(i), stlon(i), gcarc,
     &          baz, rlat1, rlon1)

** Write the piercing points into a temporary file (piercing.tmp)

        write(1,101) ph(i), dt(i), rlon1, rlat1, anisodep
101     format(5(f12.4,1x))

	enddo !! i ends. All measurements for the anisodep processed

	close(1)

** Read the splitting parameters and piercing points into arrays

	open(1,file='piercing.tmp')
	do i=1,nmea
	read(1,101,end=113) ph(i), dt(i), 
     &                      rlon(i), rlat(i)
	enddo
113	close(1)

*	We now divide the area into blocks of dx by dx in size and
*		get the mean and STD of each block, and then
*	the mean STD of all blocks as a measure of spatial coherency

** Search the area covered by the piercing points:
*  (rlata to rlatb in latitudes, and rlona to rlonb in longitudes)

	call minmax(rlat, nmea, rlatb, rlata)
	call minmax(rlon, nmea, rlonb, rlona)

	write(6,63) dx
63	format(/,'dx ===', f4.2, 'deg.')	

	write(6,62) anisodep, rlata, rlatb, rlona, rlonb
62	format('The area covered by the piercing points at ',
     &         'the depth of ', f5.1,
     &         'km is: ',/
     &          f8.3, ' to ', f8.3, 'N ', f9.3, 
     &         ' to ', f9.3, 'E')

	open(2,file='cell_mean.tmp')

	nlat=(rlatb-rlata)/dmove +1
	nlon=(rlonb-rlona)/dmove+1

** get the mean and STD for each block

	do 9903 k=1,nlat

	flat1=rlata + (k-1)*dmove
	flat2=flat1+dx

	do 9904 j=1,nlon

	flon1=rlona + (j-1)*dmove
	flon2=flon1+dx

** Open the piercing point file for this depth and get the mean for
*	those points in the area defined by flat1, flat2, flon1, and flon2

	nn=0

	do 9905 i=1,nmea

	if(rlat(i).ge.flat1.and.rlat(i).lt.flat2.and.
     &	   rlon(i).ge.flon1.and.rlon(i).lt.flon2) then
...
9905	continue !! i ends

	if(nn.ge.2) then

*       sample mean and STD for DT:
 	call meansig_sample(dtb, nn,  avdt, sigmadt1) 

*	circular mean and STD for PHI:
 	call meansig_circular2(phb, nn,  avph, sigmaph1) 

*	Combined Fv for this block:
	sigmaphdt1=  w1*sigmaph1 + w2*sigmadt1

	write(2,21) 0.5*(flon1+flon2), 0.5*(flat1+flat2), 
     &		avdt, sigmadt1, avph, sigmaph1, nn, sigmaphdt1,nin

21	format(6(f10.4, 1x), i4, 1x, f10.4,1x,a20)

	endif

9904	enddo !! j loop ends here

9903	continue!! k loop ends here

	close(2)

*** Cell averages have been computed for this discdep. 

***	Now get the mean STDs for this anisodep

	open(2,file='cell_mean.tmp')
	do i=1,maxsm
	read(2,21,end=222) flon0, flat0, avdt, tmp, avph, tmp,
     &		inn, sigmadtph(i) 
	enddo
222	close(2)
...
** Get the mean and sample STD for all the blocks for this depth
* avccc and sigccc are combined variation factors

	call meansig(sigmadtph, nn, avccc, sigccc)

        if(kk.eq.1) avccc0=avccc !! for normalization

        write(6, 81) anisodep, avccc/avccc0, sigccc/avccc0, nn,dx
        write(8, 81) anisodep, avccc/avccc0, sigccc/avccc0, nn,dx
81      format(3(f9.5, 1x), i6,1x,f4.2)

9902	continue  !! kk ends. kk is anisodep index

	close(8)

9901 	continue !! kkk loop ends. kkk is dx index
...
**----------------
        ...
        subroutine find_loc(discdep, raypi, stlat, stlon, gcarc,
     &    baz, rlat, rlon)

** Given the ray param and p or s velocity, calculates the
*	gcarc for an assumed local earthquake with depth discdep
**	Based on Stacy P 239. 
**      By S. Gao. 09/1997

*inputs:
* 1). raypi in Sec/Degree
* 2). discdep in km
* 3). vs in km/s
* 4). dep in km
	...
	parameter(rr=6371.0)
        ...
        character nin*100,  nst*9, yesno*1, nrayp*100
        real*4 vs(nmod), dep(nmod)

** Read shear-wave velocities from the IASP91 Earth model

        call readmod(dep, vs, nmod)

	pi=4.0*atan(1.0)
	rayp=raypi*180.0/pi !! change to S/rad
	yesno='y'

**	The angle of incidence. Stacy P239
*	p=r*sin(i)/v, where p is in s/rad

	gcarc=0.0

	do i=1, nmod

* angle f at the top of this layer:

	r=rr-dep(i)
        if(dep(i).ge.discdep) r=rr-discdep
	if(rayp*vs(i)/r.gt.1.0) then
		yesno='n'
		goto 321
	endif
	...
	angf=r2d*asin(rayp*vs(i)/r)
	
* angle i at the bottom of this layer:

	r=rr-dep(i+1)
        if(dep(i+1).ge.discdep) r=rr-discdep
	if(rayp*vs(i)/r. gt.1.0) then
		yesno='n'
		goto 321
	endif

	angi=r2d*asin(rayp*vs(i)/r)

* The great-circle arc correspoinding to this ray segment
	
	gcarc=gcarc+angi-angf

        if(dep(i+1).ge.discdep) goto 321
...
321	continue
        ...
        rlat=stlat+gcarc*cos(baz*d2r)
	factor=0.5*(rlat+stlat)
        rlon=stlon+gcarc*sin(baz*d2r)/cos(factor*d2r)

        end

*------------------

        subroutine readmod(dep, vs, nmod)

* Read the re-sampled IASP91 earth model
* By S.S. Gao. 09/1997

        real*4 dep(nmod), vs(nmod), tmp

        open(34,file='iasp91.6372')
        do i=1, nmod
        read(34,*,end=3232) dep(i), tmp, tmp, vs(i)
        enddo
3232    close(34)
        end
...
**----------------------------------------------------------
        ...
        subroutine minmax(data, n, fmax, fmin)
...
** By S.S. Gao
	...
	fmax=-9.9e10
        ...
        do j=1,n
        fmax=amax1(fmax, data(j))
        fmin=amin1(fmin, data(j))
        if(data(j).ge.fmax) fmax=data(j)
        if(data(j).le.fmin) fmin=data(j)
        enddo

	end

**------------------
        subroutine meansig(data,n, av,sigma)
        real*4 data(n)
        av=0.0
        do i=1,n
        av=av+data(i)
        ...
        sigma=0.0
        do i=1,n
        sigma=sigma+(data(i)-av)**2
        enddo
        sigma=1.0/sqrt(1.0*n)*sqrt(sigma/(n-1))

        end

*-------------------------

        subroutine meansig_sample(data,n, av,sigma)
	...
	goto 123
        ...
        sigma=sigma+(data(i)-av)**2
        enddo
c       sigma=1.0/sqrt(1.0*n)*sqrt(sigma/(n-1))
        sigma=1.0/sqrt(1.0*1)*sqrt(sigma/(n-1))

* see http://en.wikipedia.org/wiki/Standard_deviation for the formula
...
**----------------------
        ...
        subroutine meansig_circular2(data,n, av,sigma)

** Only for fast direcctions 
** Based on Davis 1986 
** Programmed by S.S. Gao, July, 2010
...
** Get the mean
        ...
        if(ss.ge.0.0.and.cc.ge.0.0) av=0.5/d2r*atan(ss/cc)
        if(cc.lt.0.0) av=0.5*(180.0+1.0/d2r*atan(ss/cc))
        if(ss.lt.0.0.and.cc.gt.0.0) av=0.5*(360.0+1.0/d2r*atan(ss/cc))

	flength=sqrt(ss**2 + cc**2)

** Get the STD

	s0=1-flength 

         if(flength.lt.1.0) sigma=1.0/d2r*0.5*sqrt(-2.0*log(flength))
         if(flength.ge.1.0) sigma=0.0

        end

**-----------------------------------------------------

          subroutine distazsub(evlat,evlon,stlat,
     &                      stlon,distkm,delta,baz,etos)
...
c-----Completely rewritten 04/87 by John Evans to use Bruce Julian's routines,
...
c-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
c
c#  gcen - calculate geocentric postitions, distances, and azimuths
c Original routines by Bruce R. Julian, USGS Menlo Park, CA.
c Changed to FORTRAN 77 (from ratfor) and made double precision
c by John R. Evans, 04/01/87.
c Name "geocen" changed to "gcen" to avoid confusing "make".
c
      subroutine gcen()
c
      include 'stdconst'
c
      ...
      double precision delta, az0, az1
c
      double precision st0, ct0, phi0
      save st0, ct0, phi0
      double precision st1, ct1
      double precision sdlon, cdlon
c      double precision cz0, dlon
      double precision sdelt, cdelt
      double precision dsin, dcos, datan2
c
...
c delaz - calculate the geocentric distance, azimuths
      entry delaz(lat, lon, delta, az0, az1)
      ct1 = dsin(lat)
      st1 = dcos(lat)
      sdlon = dsin(lon - phi0)
      cdlon = dcos(lon - phi0)
...
c (output - OK to overlay x, y)
      double precision r, theta
c
      double precision rtemp
      double precision datan2, hypot
c
      rtemp = hypot(x, y)
      theta = datan2(y, x)
      r = rtemp
...
c-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
c
c Routines to convert between geographic and geocentric latitude
c Bruce R. Julian, USGS Menlo Park, CA     13 Sept 1983
c DFLAT=(1 - flattening)**2
c Convert from geographic to geocentric latitude
c Changed to FORTRAN 77 (from ratfor) and made double precision
c by John R. Evans, 04/01/87.
c
      double precision function ggtogc(lat)
c
      include 'stdconst'
c
      ...
      double precision dsin, dcos, datan2
c
      ggtogc = (datan2(DFLAT*dsin(lat), dcos(lat)))
...
c
c/*         H Y P O T . R
c * Euclidian distance, accurately and avoiding overflow
c */
c Original routines by Bruce R. Julian, USGS Menlo Park, CA.
c Changed to FORTRAN 77 (from ratfor) and made double precision
c by John R. Evans, 04/01/87.
c
      double precision function hypot(a, b)
      double precision a, b
c
      double precision l, s, t
...
c
      ...
      s = s/l
      hypot = l*dsqrt(1.0 + s*s)
      return
      end
*-------------------------------------------

	subroutine tat_sks(distdg, evdep, tatp, rayp)

cccc Modified from the original IASP91 program package by Ray Buland
c to a subroutine by S. Gao on 01/12/1994
c to compute theoretical arrival times for teleseismic SKS events
c using iaspei91 travel time table.
c
	parameter(maxsta=100,maxph=60)
c variables passed to distaz subroutine
	character stname*4, event*19, outname*30,mon*2,day*2
c variables passed to tt_sub subroutine
	real*4 tt(maxph),dtdd(maxph)
	integer nph, namest
	character*8 phcd(maxph), phase_name
c local variables
	...
	call tt_sub(distdg,evdep,nph,phcd,tt,dtdd)

	do j = 1,nph
	  tatp = tt(j)
          dtdd1=dtdd(j)
	  rayp=dtdd1
	  if (phcd(j)(1:3).eq.'SKS') goto 70
	enddo
70 	continue 	
	return
	end
...
**-------------------------------------------------------
	...
	subroutine tat_skks(distdg, evdep, tatp, rayp)

cccc Modified by S. Gao from the original IASP91 program 
c    package by Ray Buland
c to compute theoretical arrival times for teleseismic SKKS events
c using iaspei91 travel time table.
c
	parameter(maxsta=100,maxph=60)
c variables passed to distaz subroutine
...
**------------------------------------------------------
	subroutine tat_pks(distdg, evdep, tatp, rayp)

cccc Modified by S. Gao from the original IASP91 program 
c    package by Ray Buland
c to compute theoretical arrival times for teleseismic PKS events
c using iaspei91 travel time table.
c
	parameter(maxsta=100,maxph=60)
c variables passed to distaz subroutine
...
**----------------------
      ...
      subroutine tt_sub(gcarc,depth,nph,phcd,tt,dtdd)
cccc Modified by S. Gao from the original IASP91 program package by Ray Buland
      save
	integer max
      parameter (max=60)
	integer nph,in,n,i
	real gcarc,depth,zs,delta
	real tt,dtdd,dtdh,dddp,mn,ts
	real usrc
      logical prnt(3)
      ...
      dimension usrc(2)
      data in/99/,modnam/'iasp91'/
      data phlst(1)/'all'/,prnt(3)/.true./
c
      prnt(1) = .false.
      prnt(2) = .false.
...
cc      call abort('Source slowness too large.')
 4    k2=i
...
c     write(msg,101)phcd(jb),x,dp0,dp,p1,p0
...
c     print *,'js phcd xbrn',js,'  ',phcd(js),xbrn(js,1)
     ...
     1 phcd(js).eq.'S').and.xbrn(js,1).le.0d0)  then  !! S. Gao 06/1998
ccc	phcd(js)='S'
	go to 1
	endif
 4    continue
...
c     write(msg,100)ptk,ptj,pti,tau,x
 100  format('Bad range: ',1p5d12.4)
      call warn(msg)
 15   if(tau.ge.-1d-10) go to 16
c     write(msg,101)ptk,ptj,pti,tau,x
...
**----------------------
	...
	subroutine baz2latlon(ala0,alo0,delt,azim,ala1,alo1,irtr)

** e.g.: from stlat, stlon, distdg, and baz to get evlat and evlon, use
**	call baz2latlon(stlat,stlon,distdg,baz,ala1,alo1,irtr)

c
c   routine to calculate latitude & longitude of point defined by
c   range and azimuth from a given starting latitude & longitude.
c   All quantities (including range) are specified in degrees.
**	Modified from a DTM subroutine. S. Gao 06/1998
c
	implicit real*8 (a-h,o-z)
	real*4 ala0,alo0,delt,azim,ala1,alo1
	data oneps/1.00001d0/
c
	irtr=0
	si=1.0d0
	pi=4.0d0*datan(si)
	...
	conv=pi/180.0d0
c
	cola0=90.0d0-dble(ala0)
	cola0=cola0*conv
	alo0r=dble(alo0)*conv
	deltr=dble(delt)*conv
	azimr=dble(azim)*conv
	if (azimr.gt.pi) si=-1.0d0
c
	cola1=dcos(cola0)*dcos(deltr)+dsin(cola0)*dsin(deltr)*dcos(azimr)
	cola1=dacos(cola1)
	alodel=dcos(deltr)-dcos(cola0)*dcos(cola1)
	alodel=alodel/(dsin(cola0)*dsin(cola1))
	if (dabs(alodel).le.1.0d0) then
		alo1r=alo0r+si*dacos(alodel)
	else if (dabs(alodel).lt.oneps) then
		if (alodel.gt.0.0d0) alo1r=alo0r
		if (alodel.lt.0.0d0) alo1r=alo0r+si*pi
	else if (dabs(alodel).ge.oneps) then
		irtr=1
		call exit()
	endif
	if (alo1r.gt.pi) alo1r=alo1r-pi2
	if (alo1r.le.-pi) alo1r=alo1r+pi2
c
	cola1=cola1/conv
	ala1=90.0d0-cola1	
	alo1=alo1r/conv
